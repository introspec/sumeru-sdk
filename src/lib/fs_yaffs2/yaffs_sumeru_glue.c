...
#ifndef HOST_SIM_MODE
    write_spi_flash(SPI_FLASH_UNIT, (char *)data, nand_chunk, data_len);
#else
...
#ifndef HOST_SIM_MODE
    read_spi_flash(SPI_FLASH_UNIT, (char *)data, nand_chunk, data_len);
#else
    int fd = (int)dev->driver_context;
    lseek(fd, nand_chunk, SEEK_SET);
			...
			int flash_erase_size, int flash_prog_size)
{
...
#ifdef HOST_SIM_MODE
	int fd;
	if ( (fd = open(HOST_SIM_FILE, O_RDWR | O_TRUNC | O_CREAT, S_IRUSR | S_IWUSR)) < 0) {
	...
	memset(buf, 0xff, 512);
	int x = 0;
	while (x < HOST_SIM_SIZE) {
	    int l = MIN(512, (HOST_SIM_SIZE - x));
	    write(fd, buf, l);
	    x += l;
	}
	free(buf);
	lseek(fd, SEEK_SET, 0);
	dev->driver_context = (void*)fd;
#endif
...
main(int argc, char **argv)
{
    ...
    if (argc < 3 || strcmp(argv[1], "-h") == 0)
        errx(1, "\n\nUSAGE:\n\n %s <image-file> <size> <op>...\n"
		"\n\t where op is one of:\n"
		"\t\t . <pathname>	\t\t: creates a directory\n"
		"\t\t <local-pathname> <pathname> \t: copies local file\n" 
		"\ne.g.,\n"
		" %s 1048576 . /sumeru /some-local-file.txt /sumeru/readme.txt\n",
		argv[0],
		argv[0]);


    HOST_SIM_FILE = argv[1];
    HOST_SIM_SIZE = strtoul(argv[2], &p, 10);
    if (argv[2][0] == 0 || *p != 0)
	errx(1, "Invalid size argument: %s\n", argv[2]);

    if ((HOST_SIM_SIZE % 65536) != 0)
	errx(1, "Size must be a multiple of 65536: %s\n", argv[2]);

    //yaffs_tracemask(1, ~0);
    if (yaffs_sumeru_devconfig("/", 0, 
			    0, (HOST_SIM_SIZE / 65536) - 1, 
			    65536, 256) < 0 || 
	yaffs_mount("/")) 
    {

...
static int yaffsfs_DoStat(struct yaffs_obj *obj, struct stat *buf)
{
	int retVal = -1;
...
#else
		buf->st_atime = obj->yst_atime;
		buf->st_ctime = obj->yst_ctime;
		buf->st_mtime = obj->yst_mtime;
#endif
		retVal = 0;
	}
	return retVal;

...
#define ASYNC_TRANSCEIVE(u,b,l) \
    spi_async_transceive(u, (char *)b, l, 0x2 | 0x4);
...
void
lcd_async_blit(unsigned int unit,
	    int x1, int y1, int x2, int y2, 
	    const char *buf, unsigned int len)
{
    if ((x1 > x2) || (y1 > y2)) {
...
void
lcd_blit_opt_async(unsigned int unit,
	    int x1, int y1, int x2, int y2, 
	    const char *buf, unsigned int len)
{
    if ((x1 > x2) || (y1 > y2)) {
    ...
    /* caller should alloc buf appropriately, buf[-1] is overwritten */
    *((char *)buf - 1) = 0x2c;
    ASYNC_TRANSCEIVE(unit, (buf - 1), (len + 1));
}

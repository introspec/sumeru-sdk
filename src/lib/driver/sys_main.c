...
setup_devices(unsigned int periph)
{
    ...
    if ((periph & CPU_PERIPH_VERSION_MASK) != CPU_PERIPH_VERSION_0x11) {
	/* XXX Improve me -- better panic / debug mechanism */
	while (1)
    ...
    /* uart0,1 and spi0,1 are fixed -- skip their setup */
    /* boot, timer, gpio are also fixed at ivectors 0,1,15 */
    ...
    dev_alloc_spi(						/* spi0 1 */
		s_DEVUNIT_SPI++,
	    	s_INTR_DISPATCH + vnr);
    ++vnr;

    dev_alloc_uart(						/* uart 1 */
		s_DEVUNIT_UART++,
	    	s_INTR_DISPATCH + vnr,
	    	s_INTR_DISPATCH + (vnr + 1));
    vnr += 2;

    /* invariant vnr == 8, unit_spi = 2, unit_uart = 2, unit_i2c = 0 */

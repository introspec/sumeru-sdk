...
#define FLASH_FIRMWARE_ADDR	(FLASH_FIRMWARE_START * FLASH_BLOCK_SIZE)

#define FLASH_FIRMWARE_MAXSIZE	\
    ((FLASH_FIRMWARE_END - FLASH_FIRMWARE_START + 1) * FLASH_BLOCK_SIZE)

#define FLASH_APP_ADDR		(FLASH_APP_START * FLASH_BLOCK_SIZE)

#define FLASH_APP_MAXSIZE	\
    ((FLASH_APP_END - FLASH_APP_START + 1) * FLASH_BLOCK_SIZE)

#define FLASH_FS_ADDR		(FLASH_ROOT_FS_START * FLASH_BLOCK_SIZE)

#define FLASH_FS_MAXSIZE	\
    ((FLASH_ROOT_FS_END - FLASH_ROOT_FS_START + 1) * FLASH_BLOCK_SIZE)
...
parse_args(int argc, char **argv)
{
	...
	case 0: 
	    ...
	    switch (lopts_index) {
	    case 0: /* flash app */
		g_flash_dst_addr = FLASH_APP_ADDR;
		g_flash_copy_mode = 1;
		...
		g_flash_dst_addr = FLASH_FIRMWARE_ADDR;
		g_flash_copy_mode = 1;
		g_input_fname = strdup(optarg);
		g_reverse_input = 1;
		break;
	    case 3: /* flash filesystem */
		g_flash_dst_addr = FLASH_FS_ADDR;
		g_flash_copy_mode = 1;
		g_input_fname = strdup(optarg);
		break;
	    case 4: /* sio terminal */
	...
	default:
		...
		"-r : Reverse input data, required for RBF (firmware) files\n"
...
static void
validate_input_maxsize(unsigned int faddr, unsigned int len)
{
    unsigned int err = 0;

    switch(faddr) {
	case FLASH_FIRMWARE_ADDR:
	    if (len > FLASH_FIRMWARE_MAXSIZE)
		err = FLASH_FIRMWARE_MAXSIZE;
	    break;
	case FLASH_APP_ADDR:
	    if (len > FLASH_APP_MAXSIZE)
		err = FLASH_APP_MAXSIZE;
	    break;
	case FLASH_FS_ADDR:
	    if (len > FLASH_FS_MAXSIZE)
		err = FLASH_FS_MAXSIZE;
	    break;
	default:
	    break;
    }
    ...
    if (err != 0)
	errx(1, "Input file is too large, it must be no " 
		"larger than %d bytes\n", err);
}
...
main(int argc, char **argv)
{
	...
	validate_input_maxsize(g_flash_dst_addr, g_input_len);
    }
